## 2026-07-15 13:00: 流量监控统一 TIME_PRESETS + 自定义日期
- **文件:** `frontend/traffic.js`, `frontend/index.html`, `frontend/style.css`
- **变更:**
  - 新增 `TIME_PRESETS` 统一预设集合（今日/昨日/近7日/本月/近30日/近90日/全部）
  - 替换流量仪表盘和明细两处硬编码 days 数组
  - 新增自定义日期选择器（支持任意 from/to 范围查询）
  - 向后兼容
- **影响范围:** 流量监控视图的时间筛选功能

## 2026-07-13 13:00: 修复流量监控"全部"选项未显示全量数据
- **文件:** `frontend/traffic.js`
- **根因:** 主面板和明细面板的"全部"发送 `days=365`，但后端原生只支持最大 365 天范围的查询，超过的数据被排除。真正的"全部"应不设上限
- **决策:** "全部"按钮值从 `365` 改为 `-1`，后端收到 `-1` 时返回所有记录（不限时间范围）
- **影响范围:** 流量监控仪表盘和流量明细两个 Tab 的"全部"按钮
